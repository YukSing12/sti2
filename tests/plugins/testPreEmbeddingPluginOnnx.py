...
emb_0 = npDataType(np.random.randn(50000,nEmbedding))
emb_1 = npDataType(np.random.randn(513,nEmbedding))
emb_2 = npDataType(np.random.randn(4,nEmbedding))
...
def layerNormCPU(bufferH):
    ...
    _wordEmb  = emb_0
    _posEmb   = emb_1
    _tokenIds = bufferH[0].astype(np.int32)
    _wordIds  = bufferH[1].astype(np.int32)
    _posIds   = bufferH[2].astype(np.int32)
...
def getLayerNormOnnx():
    ...
    tokEmb   = gs.Constant(name="tokEmb", values=emb_2)
    wordEmb  = gs.Constant(name="wordEmb", values=emb_0)
    posEmb   = gs.Constant(name="posEmb", values=emb_1)

    gamma = gs.Constant(name="gamma", values=globalGamma)
    beta = gs.Constant(name="beta", values=globalBeta)
    y = gs.Variable(name="y", dtype=idDataType, shape=shape3)
    layernorm = gs.Node(op="EmbLayerNorm", 
                        name="EmbLayerNorm_1", 
                        inputs=[tokEmb, wordEmb, posEmb, tokenIds, wordIds, posIds ], #,gamma, beta
                        outputs=[y], 
                        attrs={"epsilon":epsilon})
    graph = gs.Graph(nodes=[layernorm], inputs=[tokEmb, wordEmb, posEmb, tokenIds, wordIds, posIds], outputs=[y])#, gamma, beta
...
def run():
    ...
    # profile.set_shape(inputTensor0.name, (1, nW, nEmbedding), (4, nW, nEmbedding), (10, nW, nEmbedding))  
    ...
    profile.set_shape(inputTensor0.name, (1, nW, 1), (4, nW, 1), (10, nW, 1))  
    profile.set_shape(inputTensor1.name, (1, nW, 1), (4, nW, 1), (10, nW, 1))  
    profile.set_shape(inputTensor2.name, (1, nW, 1), (4, nW, 1), (10, nW, 1))  
    ...
    context.set_binding_shape(0,[nBS,nW,1])
    context.set_binding_shape(1,[nBS,nW,1])
    context.set_binding_shape(2,[nBS,nW,1])

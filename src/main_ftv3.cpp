...
void run(IExecutionContext* context, cudaStream_t& stream, sample& s, std::vector<void*>& vBufferH, std::vector<void*>& vBufferD) {
...
int main(int argc, char* argv[]) {
    ...
    // tmp_3
    CHECK(cudaHostAlloc(&vBufferH[3], 10 * 1 * sizeof(int), cudaHostAllocMapped));
    CHECK(cudaMalloc(&vBufferD[3], 10 * 1 * sizeof(int)));
    ...
    contexts = engine->createExecutionContext();
    contexts->setOptimizationProfileAsync(0, stream);
    contexts->setInputTensorAddress("read_file_0.tmp_0", vBufferD[0]);
    contexts->setInputTensorAddress("read_file_0.tmp_1", vBufferD[1]);
    contexts->setInputTensorAddress("read_file_0.tmp_2", vBufferD[2]);
    contexts->setInputTensorAddress("read_file_0.seq_len", vBufferD[3]);
    contexts->setInputTensorAddress("read_file_0.tmp_6-13", vBufferD[4]);
    contexts->setTensorAddress("save_infer_model/scale_0.tmp_0", vBufferD[5]);
    ...
    for (auto& s : sample_vec) {
        run(contexts, stream, s, vBufferH, vBufferD);
    }

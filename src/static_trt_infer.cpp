         ...
         const int& nBinding)
{
    ...
    // // Check binding shape
    // std::cout << std::string("Binding all? ") << std::string(context->allInputDimensionsSpecified() ? "Yes" : "No")
    // << std::endl; for (int i = 0; i < nBinding; ++i)
    // {
    //     std::cout << std::string("Bind[") << i << std::string(i < 12 ? "]:i[" : "]:o[") << (i < 12 ? i : i - 12) <<
    //     std::string("]->"); std::cout << dataTypeToString(engine->getBindingDataType(i)) << std::string(" ");

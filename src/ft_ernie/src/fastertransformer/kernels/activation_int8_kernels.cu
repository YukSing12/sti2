...
template<>
__inline__ __device__ half relu(half x)
{
#if __CUDA_ARCH__ >= 800
    return __hmax(x, __float2half(0.0f));
#else
    return __float2half(relu(__half2float(x)));
#endif
}
...
{
#if __CUDA_ARCH__ >= 800
    return __hmax2(val, __float2half2_rn(0.0f));
#else
    half2 out;
    out.x = __float2half(relu(__half2float(val.x)));
    out.y = __float2half(relu(__half2float(val.y)));
    return out;
#endif
...
// using char4 as output
// for per-channel-quantization weight
__global__ void add_bias_relu_COL32_int32I_int8O(int8_t*        out,
                                                 const int32_t* input,
                                                 const float*   bias,
                                                 const int      m,
                                                 const int      n,
...
__global__ void add_bias_relu_COL32_int32I_int8O(char4*        out,
                                                 const int4*   input,
                                                 const half2*  bias,
                                                 const int     m,
                                                 const int     n,
                            ...
                            const float*   out_scale_ptr)
{
    ...
    assert(block.x <= 1024);
    if (sizeof(T) == sizeof(half)) {
        add_bias_relu_COL32_int32I_int8O<<<grid, block, 0, stream>>>((char4*)out,
                                                                     (const int4*)in,
                                                                     (const half2*)bias,
                                                                     m,
                                                                     n,
        ...
        add_bias_relu_COL32_int32I_int8O<<<grid, block, 0, stream>>>(
            out, in, (const float*)bias, m, n, weight_amax, input_deQFactor_div127_ptr, out_scale_ptr);
    }
}

template void invokeAddBiasReluCol32(int8_t*        out,
                                     const int32_t* in,
                                     const float*   bias,
                                     const int      m,
                                     const int      n,
...
// for per-tensor-quantization weight
template<typename T>
__global__ void add_bias_relu_COL32_int8IO(int8_t*       out,
                                           const int8_t* input,
                                           const T*      bias,
                                           const int     m,
                                           const int     n,
                            ...
                            const float*  out_scale_ptr)
{
    ...
    add_bias_relu_COL32_int8IO<<<grid, block, 0, stream>>>(out, in, bias, m, n, input_deQFactor_ptr, out_scale_ptr);
}
...
// for per-tensor-quantization weight
template<typename T>
__global__ void add_bias_relu_COL32_int8IO(
    int8_t* out, const T* bias, const int m, const int n, const float* input_deQFactor_ptr, const float* out_scale_ptr)
{
                               ...
                               const float* out_scale_ptr)
{
    ...
    if (sizeof(T) == sizeof(half)) {
        add_bias_relu_COL32_int8IO<<<grid, block, 0, stream>>>(out, bias, m, n, input_deQFactor_ptr, out_scale_ptr);
    }
    else {
        add_bias_relu_COL32_int8IO<<<grid, block, 0, stream>>>(out, bias, m, n, input_deQFactor_ptr, out_scale_ptr);
    }
...
template void invokeAddBiasReluCol32_v2(int8_t*      out,
                                        const float* bias,
                                        const int    m,
                                        const int    n,
                                        cudaStream_t stream,
...
// for per-tensor-quantization weight
template<typename T>
__global__ void add_bias_relu_ROW_int8IO(int8_t*       out,
                                         const int8_t* input,
                                         const T*      bias,
                                         const int     m,
                                         const int     n,
                          ...
                          const float*  out_scale_ptr)
{
    ...
    add_bias_relu_ROW_int8IO<<<grid, block, 0, stream>>>(out, in, bias, m, n, input_deQFactor_ptr, out_scale_ptr);
}

template void invokeAddBiasReluRow(int8_t*       out,
                                   const int8_t* in,
                                   const float*  bias,
                                   const int     m,
                                   const int     n,

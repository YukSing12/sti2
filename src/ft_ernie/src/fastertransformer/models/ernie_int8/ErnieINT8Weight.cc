...
template<typename T>
void ErnieINT8Weight<T>::initialize()
{
    FT_LOG_DEBUG("ErnieINT8Weight " + std::string(__func__) + " start");
...
template<typename T>
ErnieINT8Weight<T>::~ErnieINT8Weight()
{
    FT_LOG_DEBUG("ErnieINT8Weight " + std::string(__func__) + " start");
...
template<typename T>
ErnieINT8Weight<T>::ErnieINT8Weight(const ErnieINT8Weight& other):
...
template<typename T>
ErnieINT8Weight<T>& ErnieINT8Weight<T>::operator=(const ErnieINT8Weight& other)
{
    FT_LOG_DEBUG("ErnieINT8Weight " + std::string(__func__) + " start");
...
template<typename T>
void ErnieINT8Weight<T>::setWeightPtr()
{
    FT_LOG_DEBUG("ErnieINT8Weight " + std::string(__func__) + " start");
...
template<typename T>
void ErnieINT8Weight<T>::mallocWeights()
{
    FT_LOG_DEBUG("ErnieINT8Weight " + std::string(__func__) + " start");
    for (int i = 0; i < real_weights_num_; i++) {
...
template<typename T>
void ErnieINT8Weight<T>::loadModel(std::string dir_path)
{
    FT_LOG_DEBUG("ErnieINT8Weight " + std::string(__func__) + " start");

    FtCudaDataType model_file_type = getModelFileType(dir_path + "/config.ini", "ernie");
    FT_CHECK(is_maintain_buffer == true);
    loadWeightFromBin<T>(weights_ptr[0], {(size_t)weights_size[0]}, dir_path + "/word_embedding.bin", model_file_type);
    loadWeightFromBin<T>(weights_ptr[1], {(size_t)weights_size[1]}, dir_path + "/pos_embedding.bin", model_file_type);
    loadWeightFromBin<T>(weights_ptr[2], {(size_t)weights_size[2]}, dir_path + "/sent_embedding.bin", model_file_type);
    ...
    loadWeightFromBin<T>(weights_ptr[5], {(size_t)weights_size[5]}, dir_path + "/pooled_fc.w_0.bin", model_file_type);
    loadWeightFromBin<T>(weights_ptr[6], {(size_t)weights_size[6]}, dir_path + "/pooled_fc.b_0.bin", model_file_type);
    loadWeightFromBin<T>(weights_ptr[7], {(size_t)weights_size[7]}, dir_path + "/cls_out_w.bin", model_file_type);
    loadWeightFromBin<T>(weights_ptr[8], {(size_t)weights_size[8]}, dir_path + "/cls_out_b.bin", model_file_type);

    loadWeightFromBin<T>(weights_ptr[9], {(size_t)weights_size[9]}, dir_path + "/multi_field_1.bin", model_file_type);
    loadWeightFromBin<T>(weights_ptr[10], {(size_t)weights_size[10]}, dir_path + "/multi_field_3.bin", model_file_type);
    loadWeightFromBin<T>(weights_ptr[11], {(size_t)weights_size[11]}, dir_path + "/multi_field_6.bin", model_file_type);
    loadWeightFromBin<T>(weights_ptr[12], {(size_t)weights_size[12]}, dir_path + "/multi_field_0.bin", model_file_type);
    loadWeightFromBin<T>(weights_ptr[13], {(size_t)weights_size[13]}, dir_path + "/multi_field_5.bin", model_file_type);
    loadWeightFromBin<T>(weights_ptr[14], {(size_t)weights_size[14]}, dir_path + "/multi_field_7.bin", model_file_type);
    loadWeightFromBin<T>(weights_ptr[15], {(size_t)weights_size[15]}, dir_path + "/multi_field_4.bin", model_file_type);
    loadWeightFromBin<T>(weights_ptr[16], {(size_t)weights_size[16]}, dir_path + "/multi_field_2.bin", model_file_type);
    ...
    for (int l = 0; l < num_layer_; l++) {
        ernie_encoder_layer_weights[l]->loadModel(dir_path + "/", model_file_type);
    }
    FT_LOG_DEBUG("ErnieINT8Weight " + std::string(__func__) + " end");
}

template<typename T>
void ErnieINT8Weight<T>::resizeLayer(const int num_layer)
{
    FT_LOG_DEBUG("ErnieINT8Weight " + std::string(__func__) + " start");
    ernie_encoder_layer_weights.clear();
    num_layer_ = num_layer;
...
#ifdef ENABLE_BF16
template struct ErnieINT8Weight<__nv_bfloat16>;
#endif

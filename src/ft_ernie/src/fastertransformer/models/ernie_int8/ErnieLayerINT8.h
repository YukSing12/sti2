...
template<typename T>
class ErnieLayerINT8: public BaseLayer {
private:
    // buffer handling
...
public:
    ErnieLayerINT8(size_t           max_batch_size,
                  size_t           max_seq_len,
                  size_t           head_num,
    ...
    ErnieLayerINT8(ErnieLayerINT8<T> const& ernie_layer);

    ~ErnieLayerINT8();

    void forward(std::vector<Tensor>*       output_tensors,
                 const std::vector<Tensor>* input_tensors,
                 const ErnieLayerINT8Weight<T>*  ernie_layer_weight);
};

...
#include "src/fastertransformer/models/ernie_int8/ErnieINT8Encoder.h"
#include "src/fastertransformer/utils/nvtx_utils.h"

namespace fastertransformer {

template<typename T>
void ErnieINT8Encoder<T>::initialize()
{
...
template<typename T>
ErnieINT8Encoder<T>::ErnieINT8Encoder(size_t                              max_batch_size,
                        size_t                              max_seq_len,
                        size_t                              head_num,
...
template<typename T>
ErnieINT8Encoder<T>::ErnieINT8Encoder(ErnieINT8Encoder<T> const& ernie_encoder):
...
template<typename T>
ErnieINT8Encoder<T>::~ErnieINT8Encoder()
{
...
template<typename T>
void ErnieINT8Encoder<T>::setStream(cudaStream_t stream)
{
...
template<typename T>
void ErnieINT8Encoder<T>::allocateBuffer()
{
...
template<typename T>
void ErnieINT8Encoder<T>::allocateBuffer(size_t batch_size, size_t seq_len)
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    token_num_      = (size_t*)allocator_->reMalloc(token_num_, sizeof(size_t) * 1, false);
...
template<typename T>
void ErnieINT8Encoder<T>::freeBuffer()
{
...
template<typename T>
bool ErnieINT8Encoder<T>::isValidLayerParallelId(uint l)
{
...
template<typename T>
bool ErnieINT8Encoder<T>::isFirstLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
    return l < num_layer_ && (l == local_num_layer * pipeline_para_.rank_);
}
...
template<typename T>
bool ErnieINT8Encoder<T>::isLastLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
int ErnieINT8Encoder<T>::getFirstLayerParallelId()
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
void ErnieINT8Encoder<T>::forward(std::vector<Tensor>*       output_tensors,
                           const std::vector<Tensor>* input_tensors,
                           const ErnieINT8EncoderWeight<T>*  ernie_encoder_weights)
{
    // input_tensors:
...
template<typename T>
void ErnieINT8Encoder<T>::forward(std::unordered_map<std::string, Tensor>*       output_tensors,
                           ...
                           const ErnieINT8EncoderWeight<T>*                      ernie_encoder_weights)
{
    ...
    const ErnieINT8EncoderLayerWeight<T>* ernie_layer_int8_weight = (const ErnieINT8EncoderLayerWeight<T>*)ernie_encoder_weights;
...
template class ErnieINT8Encoder<float>;
template class ErnieINT8Encoder<half>;
// #ifdef ENABLE_BF16
// template class ErnieINT8Encoder<__nv_bfloat16>;
// #endif

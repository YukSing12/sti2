...
void ErnieEncoder<T>::initialize()
{
    ...
    if ((attention_type_ == AttentionType::FUSED_MHA || attention_type_ == AttentionType::FUSED_PADDED_MHA)
        ...
        for(size_t i=0;i<num_layer_;i++)
            attention_layer_[i] = new FusedAttentionLayer<T>(max_batch_size_,
                                                        max_seq_len_,
                                                        head_num_,
                                                        size_per_head_,
                                                        d_model_,
    ...
    if (activation_type_ == ActivationType::Gelu || activation_type_ == ActivationType::GeGLU) {
        for(size_t i=0;i<num_layer_;i++)
            ffn_layer_[i] = new TensorParallelGeluFfnLayer<T>(max_batch_size_,
                                                        max_seq_len_,
                                                        1,
                                                        d_model_,
                                                        inter_size_,
        ...
        for(size_t i=0;i<num_layer_;i++)
            ffn_layer_[i] = new TensorParallelReluFfnLayer<T>(max_batch_size_,
                                                        max_seq_len_,
                                                        1,
                                                        d_model_,
                                                        inter_size_,
        ...
        for(size_t i=0;i<num_layer_;i++)
            ffn_layer_[i]  = new TensorParallelSiluFfnLayer<T>(max_batch_size_,
                                                       max_seq_len_,
                                                       1,
                                                       d_model_,
                                                       inter_size_,
...
void ErnieEncoder<T>::allocateBuffer()
{
    if (is_allocate_buffer_ == false) {
        ...
        ernie_layer_out_buffer_ = (T*)allocator_->reMalloc(
            ernie_layer_out_buffer_, sizeof(T) * max_batch_size_ * max_seq_len_ * d_model_, false);
                           ...
                           const ErnieEncoderWeight<T>*                      ernie_encoder_weights)
{
   ...
   // allocateBuffer(request_batch_size, request_seq_len);
    ...
    const int* sequence_lengths = input_tensors->at("seq_len").getPtr<int>();
    ...
    invokeEmbeddingLookupConcat(ernie_encoder_emb_buf_,
                                ...
                                input_tensors->at("sent_ids").getPtr<int>(),

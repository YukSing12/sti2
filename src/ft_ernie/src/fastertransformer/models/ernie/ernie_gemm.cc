...
int main(int argc, char* argv[])
{
    if (argc != 14 && argc != 15 && argc != 16 && argc != 17) {
        FT_LOG_ERROR("[ERROR] ./bin/ernie_gemm  \\ \n"
                     "              batch_size \\ \n"
                     "              beam_width \\ \n"
                     "              max_mem_seq_len \\ \n"
                     "              encoder_d_model \\ \n"
                     ...
                     "              is_append");
        FT_LOG_ERROR("e.g. ./bin/ernie_gemm 8 4 32 512 8 64 2048 512 8 64 2048 32100 1 2 0 0");
        return 0;
    }
    ...
    std::cout << "[INFO] arguments: " << std::endl
              ...
              << "    is_append:" << is_append << std::endl;
    void*  gemm_test_buf;
    size_t buf_size_in_byte = ft::calErnieGemmTestBufSizeInByte(batch_size,
                                                             beam_width,
                                                             max_mem_seq_len,
                                                             encoder_d_model,
                                                             encoder_head_num,
    ...
    if (data_type == ft::FLOAT_DATATYPE) {
        ft::generate_ernie_gemm_config<float>(batch_size,
                                           beam_width,
                                           max_mem_seq_len,
                                           encoder_d_model,
                                           encoder_head_num,
        ...
        ft::generate_ernie_gemm_config<half>(batch_size,
                                          beam_width,
                                          max_mem_seq_len,
                                          encoder_d_model,
                                          encoder_head_num,
        ...
        ft::generate_ernie_gemm_config<__nv_bfloat16>(batch_size,
                                                   beam_width,
                                                   max_mem_seq_len,
                                                   encoder_d_model,
                                                   encoder_head_num,
